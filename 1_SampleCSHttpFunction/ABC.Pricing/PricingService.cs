...
namespace SampleCSHttpFunction.ABC.Pricing
{
    class PricingService
    {
        ...
        public static PricingResponse Calculate(PricingRequest request, ILogger log)
        {
            ...
            if (String.IsNullOrEmpty(request.UserRole))
            {
                return CreateErrorResponse("User if not able to order product.", request.ModelNumber);
            }
            ...
            {
                // log the exception
                log.LogCritical("Unknown error occurred with Pricing request", request, ex);
